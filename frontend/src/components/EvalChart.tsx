...
export function EvalChart({ evaluations, currentPly, onSelect }: EvalChartProps) {
  return (
        ...
        {evaluations.map((value, index) => {
          ...
          return (
            <button
              key={`${index}-${value}`}
              type="button"
              className={`eval-bar ${isPositive ? 'eval-bar--positive' : 'eval-bar--negative'} ${
                isActive ? 'eval-bar--active' : ''
              }`}
              style={{ height: `${height}px` }}
              onClick={() => onSelect(index)}
              title={`Coup ${index}: ${formatEval(value)}`}
            >
