...
function getDisplayedFiles(orientation) {
    return orientation === 'white' ? [...FILES] : [...FILES].reverse();
}

function getDisplayedRanks(orientation) {
...
function getSquareCenter(square, orientation) {
    ...
    const xIndex = files.indexOf(square[0]);
    const yIndex = ranks.indexOf(square[1]);
    return {
        x: (xIndex + 0.5) * 12.5,
        y: (yIndex + 0.5) * 12.5,
    };
...
function buildSquareStyle({ square, selectedSquare, officialLastMoveSquares, playedMoveSquares, playedMoveColor, bestMoveSquares, showBestMove, }) {
    ...
    if (playedMoveSquares?.from === square) {
        backgrounds.push(`radial-gradient(circle, ${colorToRgba(playedMoveColor, 0.22)}, ${colorToRgba(playedMoveColor, 0.1)})`);
        shadows.push(`inset 0 0 0 2.5px ${colorToRgba(playedMoveColor, 0.58)}`);
    }

    if (playedMoveSquares?.to === square) {
        backgrounds.push(`radial-gradient(circle, ${colorToRgba(playedMoveColor, 0.26)}, ${colorToRgba(playedMoveColor, 0.14)})`);
    ...
    if (showBestMove && bestMoveSquares?.to === square && !isPlayedTo) {
        backgrounds.push(`radial-gradient(circle, ${colorToRgba(BEST_MOVE_COLOR, 0.2)}, ${colorToRgba(BEST_MOVE_COLOR, 0.08)})`);
...
function buildArrowGeometry(from, to, orientation, shortenBy = 3.2) {
...
function getPieceStyle(piece, isDragging) {
    ...
    return {
        ...
        lineHeight: 1,
        fontWeight: 700,
        fontFamily: '"Segoe UI Symbol", "Arial Unicode MS", "Noto Sans Symbols 2", "Apple Symbols", sans-serif',
...
export function ChessBoardPanel({ game, currentPly, boardFen, boardOrientation, onPieceDrop, onFlip, onPrev, onNext, showBestMove, onToggleBestMove, playedMoveReview, positionReview, sandboxFeedback, isSandboxActive, onResetSandbox, }) {
    ...
    const displayedSquares = useMemo(() => displayedRanks.flatMap((rank) => displayedFiles.map((file) => `${file}${rank}`)), [displayedFiles, displayedRanks]);
    ...
    const renderedArrows = useMemo(() => {
        ...
        const isSameAsPlayed = bestMoveData.squares &&
            playedMoveSquares &&
    ...
    const handleMoveAttempt = (sourceSquare, targetSquare) => {
    ...
    return (
        <section className="panel board-panel">
            ...
            <div className="board-shell">
                <div className="analysis-board-frame" style={{
                    borderRadius: 28,
                    padding: 14,
                    ...
                    <svg className="analysis-board-arrows" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true" style={{
                        position: 'absolute',
                        inset: 0,
                        width: '100%',
                        height: '100%',
                        ...
                        overflow: 'visible',
                            ...
                            <defs>
                            {renderedArrows.map((arrow, index) => (<marker key={`${arrow.key}-marker`} id={`analysis-arrow-${index}`} markerWidth="5" markerHeight="5" refX="4.1" refY="2.5" orient="auto" markerUnits="strokeWidth">
                            ...
                            {renderedArrows.map((arrow, index) => {
                                ...
                                <circle cx={geometry.start.x} cy={geometry.start.y} r="0.9" fill={colorToRgba(arrow.color, Math.min(arrow.opacity, 0.86))}/>
                                <line x1={geometry.start.x} y1={geometry.start.y} x2={geometry.end.x} y2={geometry.end.y} stroke={colorToRgba(arrow.color, arrow.opacity)} strokeWidth={arrow.strokeWidth} strokeLinecap="round" strokeDasharray={arrow.dashed ? '3 2' : undefined} markerEnd={`url(#analysis-arrow-${index})`}/>
                                </g>);
                            })}
                        </svg>

                        <div className="analysis-board" role="grid" aria-label="Échiquier d'analyse" style={{
                            position: 'relative',
                            zIndex: 2,
                            width: '100%',
                            height: '100%',
                            ...
                            return (<button key={`${square}-${piece ?? 'empty'}`} type="button" className={`analysis-square ${isDarkSquare(square) ? 'analysis-square--dark' : 'analysis-square--light'}`} style={squareStyle} onClick={() => handleSquareClick(square)} onDragOver={(event) => {
                                if (dragSource) {
                                ...
                                {piece ? (<span className={`analysis-piece analysis-piece--${pieceColorClass} ${dragSource === square ? 'analysis-piece--dragging' : ''}`} style={getPieceStyle(piece, dragSource === square)} draggable={piece[0]?.toLowerCase() === turnColor} onDragStart={(event) => {
            ...
            {sandboxFeedback ? (<div className={`sandbox-card sandbox-card--${sandboxFeedback.category}`}>
                <div>
                ...
                {isSandboxActive ? (<button type="button" className="btn btn--secondary" onClick={onResetSandbox}>
                    Back to the game
                </button>) : null}
            </div>) : null}
        </section>
