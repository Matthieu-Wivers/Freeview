...
export function MovesSidebar({ game, review, currentPly, currentEval, currentMoveIndex, sandboxFeedback, onJumpToPly, }) {
    ...
    <aside className="sidebar-layout">
      <section className="panel move-review-panel">
        ...
        {currentMoveReview ? (<div className="review-card">
      ...
      <section className="panel move-list-panel">
          ...
          {Array.from({ length: Math.ceil(game.moves.length / 2) }).map((_, rowIndex) => {
            ...
            return (
              <div key={rowIndex} className="moves-row">
                ...
                {whiteMove ? (<button type="button" className={`move-chip ${currentMoveIndex === whiteMove.ply ? 'move-chip--active' : ''}`} onClick={() => onJumpToPly(whiteMove.ply)}>
                ...
                {blackMove ? (<button type="button" className={`move-chip ${currentMoveIndex === blackMove.ply ? 'move-chip--active' : ''}`} onClick={() => onJumpToPly(blackMove.ply)}>
                    <span className="move-chip__text">
        ...
        {currentPositionSuggestion ? (<div className="review-card review-card--suggestion" style={{ display: "none" }}>
            <div className="review-card__header">
              <div>
                <span className="meta-label">Meilleur coup dans la position affichée</span>
                <h3>{currentPositionSuggestion.bestSan}</h3>
              </div>
              <ReviewBadge category={currentPositionSuggestion.category} label={currentPositionSuggestion.label}/>
            </div>
        ...
        {sandboxFeedback ? (<div className="inline-note">
