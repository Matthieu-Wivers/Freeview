...
function pieceSquareBonus(type: string, color: ChessColor, row: number, col: number): number {
...
function countMobility(fen: string): { white: number; black: number } {
...
export function evaluatePositionWhite(fen: string): number {
  ...
  if (placement.includes('K')) {
    if (placement.includes('K') && (currentFen.includes(' w ') || currentFen.includes(' b '))) {
      ...
      if (whiteCastled) score += 0.3;
      if (blackCastled) score -= 0.3;
    }
  }
...
function terminalScore(chess: Chess): number {
...
function minimaxWhite(fen: string, depth: number, alpha: number, beta: number, cache: Map<string, number>): number {
  const key = `${fen}|${depth}`;
...
export function scorePositionMoves(fen: string, depth = ENGINE_DEPTH): CandidateMove[] {
  const chess = createChessFromFen(fen);
  const legalMoves = chess.moves({ verbose: true }) as Array<{
    san: string;
    from: string;
...
export function classifyMove(loss: number, ply: number): ReviewCategory {
  if (ply <= 10 && loss <= 0.12) return 'theory';
  if (loss <= 0.08) return 'best';
  if (loss <= 0.28) return 'excellent';
  if (loss <= 0.65) return 'good';
  if (loss <= 1.2) return 'inaccuracy';
...
export function analyzeGame(game: ParsedGame): GameReview {
  const moveReviews = game.moves.map((move) =>
    buildMoveReview(move.ply, move.fenBefore, move.fenAfter, move.san, move.uci),
  );
...
export function analyzeSandboxMove(fen: string, uci: string, san: string): SandboxFeedback {
