import { CATEGORY_ACCURACY, CATEGORY_COMMENTS, CATEGORY_LABELS, PIECE_VALUES, } from './constants';
...
function classifyMove(args) {
    const { ply, loss, isExactBest, theoryStillOpen, moverMaterialBefore, moverMaterialAfter, bestGap, bestScoreForMover, actualScoreForMover, } = args;
    ...
    if (isExactBest || loss <= 0.08)
        return 'excellent';
    if (loss <= 0.22)
        return 'good';
    if (loss <= 0.55)
        return 'inaccuracy';
    if (loss <= 1.5 || actualScoreForMover > -3)
        return 'mistake';
    return 'blunder';
}
