...
export function parsePgn(rawPgn = '') {
  ...
  const moves = history.map((entry, index) => {
    ...
    if (!applied) {
      throw new Error(`Unable to replay move ${entry.san}.`);
    }

    const san = normalizeSanForDisplay ? normalizeSanForDisplay(entry.san) : entry.san;
...
export function summarizePgn(rawPgn = '') {
  ...
  try {
    ...
    return {
      isValid: true,
      error: null,
      title: headers.Event || 'Untitled game',
      event: headers.Event || 'Untitled game',
      site: headers.Site || '',
      date: headers.Date || '',
      round: headers.Round || '',
    ...
    return {
      ...
      title: headers.Event || 'Invalid game',
      event: headers.Event || 'Invalid game',
      site: headers.Site || '',
      date: headers.Date || '',
      round: headers.Round || '',
      white: headers.White || 'White',
